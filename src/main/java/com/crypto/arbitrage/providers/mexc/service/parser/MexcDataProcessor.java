...
public class MexcDataProcessor {
    ...
    private Map<String, InstrumentInfo> knowInstruments = new HashMap<>();
    ...
    public void onInstrumentEvent(@NonNull MexcInstrumentEvent event) {
        if (event instanceof MexcSubscribedInstrumentEvent subscribedInstrumentEvent) {
            knowInstruments.put(subscribedInstrumentEvent.getInstrumentInfo().symbol, subscribedInstrumentEvent.getInstrumentInfo());
    ...
    private OrderInfo getOrderInfo(MexcOrderResponse mexcOrderInfoResponse) {
        ...
        int filled = (int) (mexcOrderInfo.getCumulativeQuantity().doubleValue() * instrumentInfo.sizeMultiplier);
        int remaining = (int) (mexcOrderInfo.getRemainQuantity().doubleValue() * instrumentInfo.sizeMultiplier);
        boolean isBid = isBid(mexcOrderInfo.getTradeType());
        OrderType orderType = getOrderType(mexcOrderInfo.getOrderType());

        OrderInfoBuilder orderInfoBuilder = new OrderInfoBuilder(symbol, mexcOrderInfo.getOrderId(), isBid, orderType, mexcOrderInfo.getClientOrderId(), false);
