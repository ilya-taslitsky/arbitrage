...
public class CEXEngine implements Layer1ApiDataListener, Layer1ApiInstrumentListener {
  @Getter private volatile int bidPrice;
  @Getter private volatile int askPrice;
  ...
  public void initialize(
      DatasourceInfo dataSourceInfo, Map<String, InstrumentInfo> aliasToInstrumentInfo) {
  ...
  public void onDepth(String alias, boolean isBid, int price, int size) {
    InstrumentInfo instrumentInfo = aliasToInstrumentInfo.get(alias);
    if (instrumentInfo == null || !instrumentInfo.symbol.equals(this.alias)) return;
  ...
  private void updateOrderBookBbo() {
    if (orderBook.getBidMap().isEmpty() || orderBook.getAskMap().isEmpty()) return;

    int bidPrice = orderBook.getBestBidPriceOrNone();
    int askPrice = orderBook.getBestAskPriceOrNone();

    if (bidPrice == Integer.MAX_VALUE
        || askPrice == Integer.MAX_VALUE
        || bidPrice > askPrice
        || (askPrice - bidPrice) > 20) {
      return;
    }
