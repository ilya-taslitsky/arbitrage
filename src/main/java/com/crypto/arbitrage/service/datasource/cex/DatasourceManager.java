package com.crypto.arbitrage.service.datasource.cex;
...
public class DatasourceManager implements ApplicationContextAware {
  ...
  private final Map<String, ProviderContext<?>> providerContextCache = new HashMap<>();
  private final Map<String, DataSourceLocal> dataSourceLocalCache = new HashMap<>();
  private final Map<Layer1ApiProvider, Map<String, Integer>> providerToInstumentCountSubscribers =
      new HashMap<>();
  private final Map<String, Map<String, InstrumentInfo>> providerToAliasInstrumentInfo =
      new HashMap<>();
  ...
  private void processActivation(DatasourceInfo dataSourceInfo, boolean isRegister) {
    ...
    Map<String, Integer> instumentToSubCountMap =
        providerToInstumentCountSubscribers.get(getProvider(dataSourceInfo.getExchange()));
    ...
    if (isRegister) {
      dataSource.subscribe(
          dataSourceInfo,
          subCount,
          providerToAliasInstrumentInfo.get(dataSourceInfo.getExchange()));
      instumentToSubCountMap.put(instrument, ++subCount);
    } else {
      dataSource.unSubscribe(
          dataSourceInfo.getExchangePortAlias(),
          dataSourceInfo.getExchangePortExchange(),
          dataSourceInfo.getExchangePortType(),
          subCount);
  ...
  public void stopDatasource(Long id) {
    ...
    DatasourceInfo dataSourceInfo =
        optionalDataSourceInfo.orElseThrow(
